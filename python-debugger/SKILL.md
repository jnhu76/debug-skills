...
description: Debug Python runtime and logic bugs using pdb or ipdb. Use this for wrong output, tracebacks, stale callsites, object-shape drift, async issues, import confusion, and runtime state mismatches.
metadata:
...
- wrong output
- object or dict shape drift
- import or path confusion
- async state bugs
...
Look for `pyproject.toml`, `requirements.txt`, `poetry.lock`, `uv.lock`, `.venv`, `venv`, CLI entry scripts, and failing tests. Reuse the existing project environment whenever possible. Do not invent a global environment.
...
- restore expected object shape
- repair caller and callee contract together
- correct None handling or bad defaults
- fix import resolution
- restore async sequencing assumptions
- snapshot and restore edited files if a speculative fix fails

---
name: go-debugger
description: Debug Go services, workers, tests, and CLIs using Delve. Use this for panic analysis, wrong output, stale callsites, interface-nil traps, channel behavior, goroutine issues, and state mismatches.
metadata:
...
- Go panic investigation
- wrong output in services or workers
- goroutine and channel issues
- interface nil traps
- stale callsites after refactor
...
Look for `go.mod`, `go.sum`, `cmd/`, `main.go`, failing `_test.go` files, and service entrypoints. Prefer `go test` or the smallest runnable path before broad scanning.
...
- interface_nil_trap
- shape_mismatch
- goroutine_order_issue
- channel_behavior_error
- bad_default
- unknown
...
- repair caller and callee contracts together
- restore interface and struct shape discipline
- fix goroutine coordination assumptions
- fix channel protocol expectations
- restore safe state before continuing after a failed edit

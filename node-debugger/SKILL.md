---
name: node-debugger
description: Debug Node.js runtime behavior using node inspect or IDE debuggers. Use this for wrong output, exceptions, async flow problems, promise chain issues, closure state bugs, event loop issues, and runtime shape drift.
metadata:
...
- wrong output
- async flow and promise chain issues
- closure state confusion
- event loop related problems
...
Look for `package.json`, lockfiles, npm scripts, likely CLI or service entry files, and failing test files. Prefer the project package manager and scripts rather than inventing new commands.
...
- repair async sequencing assumptions
- restore object shape expectations
- fix stale callsites or closure capture issues
- revert speculative edits that widen the blast radius
